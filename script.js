document.addEventListener('DOMContentLoaded', () => {  // <-- Envuelve todo el código aquí
  ...
  function evaluarTodo() {
      ...
      puntuacionFinal = Math.max(0, puntuacionFinal); // Evita puntuaciones negativas
  ...
  function calcularPuntuacionPregunta(numeroPregunta) {
      try {
          // Selector más robusto y verificación de existencia
          const selectElement = document.querySelector(`#pregunta${numeroPregunta} select`); 
          if (selectElement) {
              let respuesta = selectElement.value;
              let puntuacion = 0;
              ...
              console.error(`No se encontró el select para la pregunta ${numeroPregunta}`);
              return 0; // O un valor por defecto
          }
      } catch (error) {
          console.error(`Error en calcularPuntuacionPregunta(${numeroPregunta}):`, error);
          return 0;
      }
  }
  ...
  if (signupButton) { // Check if the button exists before adding the listener
      signupButton.addEventListener('click', validarFormulario);
  } else {
      console.error("Signup button not found!");
  }

  const evaluateButton = document.getElementById('button'); // Get the button by ID
  if (evaluateButton) {
      evaluateButton.addEventListener('click', evaluarTodo);
  } else {
